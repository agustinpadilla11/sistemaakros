    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-4 rounded-xl border border-slate-200 shadow-sm gap-4">
        <div className="flex items-center gap-3">
          <div className="bg-purple-100 w-10 h-10 rounded-full flex items-center justify-center overflow-hidden border-2 border-white shadow-sm shrink-0">
          ...
          <h1 className="text-sm font-bold uppercase tracking-tight truncate">{alumna.nombre_completo}</h1>
        </div>
        <div className="flex items-center gap-4 w-full sm:w-auto">
          <button 
            onClick={() => setShowEditModal(true)}
            className="flex-1 sm:flex-none bg-emerald-100 text-emerald-700 px-4 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-emerald-200 transition-colors flex items-center justify-center gap-2"
          >
            <Edit3 className="w-3.5 h-3.5"/> Actualizar Datos
          </button>
          <Link to="/portal" className="text-[10px] font-black text-slate-400 uppercase tracking-widest hover:text-purple-600 underline">Volver</Link>
        </div>
      </div>


      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 lg:col-span-1 border-t-4 border-t-purple-600 space-y-6">
          <div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 lg:col-span-2 space-y-8">
          <div>
            <h2 className="text-sm font-bold uppercase tracking-tight mb-4 flex items-center justify-between border-b border-slate-100 pb-2">
              Apto Médico
              {aptoStatus === 'vigente' && <span className="bg-emerald-100 border-emerald-200 border text-emerald-700 px-3 py-1 rounded text-[10px] flex items-center gap-1 shrink-0"><CheckCircle className="w-3 h-3"/> Vigente</span>}
              {aptoStatus === 'vencer' && <span className="bg-amber-100 border-amber-200 border text-amber-700 px-3 py-1 rounded text-[10px] flex items-center gap-1 shrink-0"><AlertCircle className="w-3 h-3"/> Vence pronto</span>}
              {aptoStatus === 'vencido' && <span className="bg-red-100 border-red-200 border text-red-700 px-3 py-1 rounded text-[10px] flex items-center gap-1 shrink-0"><XCircle className="w-3 h-3"/> Vencido</span>}
              {aptoStatus === 'falta' && <span className="bg-slate-100 border border-slate-200 text-slate-500 px-3 py-1 rounded text-[10px] flex items-center gap-1 shrink-0"><AlertCircle className="w-3 h-3"/> No entregado</span>}
            </h2>
            <p className="text-xs font-bold uppercase text-slate-500 mb-4 leading-relaxed">
            ...
            <button className="bg-purple-100 text-purple-700 px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-purple-200 transition-colors mt-2">
